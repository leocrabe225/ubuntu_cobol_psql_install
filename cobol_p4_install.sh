...
cp pg_hba_trust.conf /etc/postgresql/15/main/pg_hba.conf
...
cp pg_hba_final.conf /etc/postgresql/15/main/pg_hba.conf
sudo systemctl restart postgresql
...
./run
echo "Please not that the postgres db user password is 'mdp', and should be changed"